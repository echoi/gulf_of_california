...
int main()
{
    ...
    const int num_phase = 5;
  
    std::ofstream initC_file;
    initC_file.open("goc_initC_Baja_Interface.txt");
    initC_file << "# POINTS: " << nx <<" "<< nz << std::endl;
    initC_file << "# Columns: x y Mafic_Baja_crust Felsic_Baja_crust NA_crust NA_mantle Interface" << std::endl;
    for( int j = 0; j < nz; ++j ) {
        for (int i = 0; i < nx; ++i) {
            ...
            // If in the plate interface: 
            // 2 km-thick from the bottom of the NA plate and the wedge and from trench to x = 500 km. 
            if( (xn > xt && xn < 500.0e3) && std::abs( (zmax - zn) - ((xn - xt) * tan_delta + 1.0e3)  ) <= 1.0e3 )
                compositions[4] = 1.0;

            // Write the compoistion filed at the current position
            initC_file << xn <<" "<< zn;
