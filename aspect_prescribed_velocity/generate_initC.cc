    ...
    for( int j = 0; j < nz; ++j ) {
        for (int i = 0; i < nx; ++i) {
            ...
            if( xn > xt && (zmax - zn) < ( xn - xt ) * tan_delta ) {
                // if in the NA upper crust
                if( zn > zmax - 15.0e3 ) {
                    compositions[0] = 1.0;
                    std::cerr << "(0) " << xn <<" "<< zn <<" "<< compositions[0]<<" "<< compositions[1] <<" "<< compositions[2] << std::endl;
                }
                // if in the NA lower crust
                else if( zn > zmax - 35.0e3 ) {
                    compositions[1] = 1.0;
                    std::cerr << "(1) " << xn <<" "<< zn <<" "<< compositions[0]<<" "<< compositions[1] <<" "<< compositions[2] << std::endl;
                }
                // if in the NA mantle
                else {
                    compositions[2] = 1.0;
                    std::cerr << "(2) " << xn <<" "<< zn <<" "<< compositions[0]<<" "<< compositions[1] <<" "<< compositions[2] << std::endl;
                }
            }
            initC_file << xn <<" "<< zn;
